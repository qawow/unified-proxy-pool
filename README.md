...
| `exit_via` 配置无法解析 | **fail-closed：拒绝拨号并报错**，不会退回「绕开 VPS 直接从免费代理出网」。前置代理写错时出口整体不可用，而不是静默泄露真实出口 |
| `exit_via` 的协议 | 只接受 `http://`、`socks5://`（`socks`/`socks5h` 归一到 socks5）、`socks4://`。`https://`（到代理这一跳裹 TLS）未实现，**直接报错而非降级为明文**；其余协议按不支持报错 |
| `exit_via` 这一跳的评分 | 属于用户配置而非池成员，链式失败时永不扣分；失败只归因到真正断掉的那一跳 |
| 配了 `exit_via` 时的校验探测 | **经前置节点探测**：entry 模式按 `[VPS → 候选]`、exit 模式按 `[候选 → VPS]` 走真实链式拨号，保证「能进池 = 能在实际链里用」；前置挂了则探测跳过不计分（候选不被误删），30s 冷却后才重试前置 |
